...
unsigned long delayMicrosecondsx(int k) {
...
void QTRSensorsRC::readPrivate(unsigned int *sensor_values)
{
    ...
    delayMicrosecondsx(10);              // charge lines for 10 us
    ...
    unsigned long startTime = myMicros();
    while (myMicros() - startTime < _maxValue)
    {
        unsigned int time = myMicros() - startTime;
        for (i = 0; i < _numSensors; i++)
        {
